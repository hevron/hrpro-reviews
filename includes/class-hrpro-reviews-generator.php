...
class HRPro_Reviews_Generator {
	...
	public function bf_post_class( $classes ) {
		...
		if ( is_single( get_the_ID() ) && HRPro_Reviews::get_meta( '_bs_review_pos' ) && HRPro_Reviews::get_meta( '_bs_review_pos' ) != 'none' ) {
			$classes[] = 'review-post-' . HRPro_Reviews::get_meta( '_bs_review_pos' );
		}
	...
	public function bf_main_content( $content ) {
		...
		if ( $atts['position'] && $atts['position'] != 'none' ) {
			...
			if ( function_exists( 'is_hrpro_amp' ) && is_hrpro_amp() ) {

				static $amp_style_preinted;

				if ( ! $amp_style_preinted ) {
					ob_start();
					include bf_append_suffix( HRPro_Reviews::dir_path( 'css/hrpro-reviews' ), '.css' );

					if ( is_rtl() ) {
						include bf_append_suffix( HRPro_Reviews::dir_path( 'css/hrpro-reviews-rtl' ), '.css' );
					}

					hrpro_amp_add_inline_style( hrpro_amp_css_sanitizer( ob_get_clean() ), 'hrpro-reviews' );
				}
			}
	...
	public function prepare_atts( $atts = array() ) {
		...
		if ( ! isset( $atts['type'] ) ) {
			$atts['type'] = HRPro_Reviews::get_meta( '_bs_review_rating_type' );
		}

		if ( ! isset( $atts['heading'] ) ) {
			$atts['heading'] = HRPro_Reviews::get_meta( '_bs_review_heading' );
		}

		if ( ! isset( $atts['verdict'] ) ) {
			$atts['verdict'] = HRPro_Reviews::get_meta( '_bs_review_verdict' );
		}

		if ( ! isset( $atts['summary'] ) ) {
			$atts['summary'] = HRPro_Reviews::get_meta( '_bs_review_verdict_summary' );
		}

		if ( ! isset( $atts['criteria'] ) ) {
			$atts['criteria'] = HRPro_Reviews::get_meta( '_bs_review_criteria' );
		}

		if ( ! isset( $atts['position'] ) ) {
			$atts['position'] = HRPro_Reviews::get_meta( '_bs_review_pos' );
		}

		if ( ! isset( $atts['extra_desc'] ) ) {
			$atts['extra_desc'] = HRPro_Reviews::get_meta( '_bs_review_extra_desc' );
		}
	...
	public function generate_block( $atts, $prepared_atts = FALSE ) {
		...
		?>
	<section class=" hrprodesign-review type-<?php echo $atts['type'], ' ', $atts['class']; ?>">
		...
		<ul class="criteria-list"><?php
			...
			foreach ( $atts['criteria'] as $criteria ) {
				...
				<li class="clearfix">
					<div class="criterion">
					<span
						class="title"><?php echo ! empty( $criteria['label'] ) ? $criteria['label'] : __( 'Criteria', 'perla' ); ?></span>
